...
export default function AboutSection() {
  ...
  const [showEthSwap, setShowEthSwap] = useState<boolean>(false);
  ...
  const dialogSequence = useMemo(
    () => [
      ...
      "",
      "> END TRANSMISSION",
      "> [ SWAP & BRIDGE PORTALS ACTIVE BELOW ]",
    ],
    []
  );
  ...
  return (
    ...
    >
      <div className="max-w-5xl w-full mx-auto px-4 md:px-8 relative z-20">
          ...
          >
            ...
            {currentStep >= dialogSequence.length && (
              <div className={`action-buttons ${isPulsing ? "active" : ""}`}>
                <button
                  onClick={() => setShowEthSwap(!showEthSwap)}
                  className="action-button pepu-button"
                >
                  <span className="button-text">{showEthSwap ? "Hide Swap ▲" : "Buy PEPU ▼"}</span>
                </button>
                <button
                  onClick={() => setShowBridge(!showBridge)}
                  className="action-button bridge-button"
                >
                  <span className="button-text">{showBridge ? "Hide Bridge ▲" : "Bridge PEPU ▼"}</span>
                </button>
              </div>
            )}
          </div>
        ...
        {showEthSwap && (
          <div id="eth-swap-section">
            <EthSwap 
              isVisible={currentStep >= dialogSequence.length && isPulsing} 
              onClose={() => setShowEthSwap(false)}
            />
          </div>
        )}
