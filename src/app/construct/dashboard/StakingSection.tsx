...
export default function StakingSection() {
  ...
  const { switchChain } = useSwitchChain(); // Hook to trigger network switch
  ...
  // Enable data fetching ONLY if connected AND on the correct network
  const { data: mfgBalanceData, refetch: refetchMfgBalance } = useReadContract({
    ...
    args: address ? [address] : undefined, enabled: isConnected && isCorrectNetwork,
  });
  ...
  const { data: userStakeData, refetch: refetchUserStake } = useReadContract({
    ...
    args: [POOL_ID, address], enabled: isConnected && isCorrectNetwork,
  });
  ...
  const { data: poolData } = useReadContract({
    ...
    args: [POOL_ID], enabled: isConnected && isCorrectNetwork,
  });

  const { data: allowanceData, refetch: refetchAllowance } = useReadContract({
    address: MFG_TOKEN_ADDRESS, abi: ERC20_ABI, functionName: "allowance",
    args: address ? [address, STAKING_CONTRACT_ADDRESS] : undefined, enabled: isConnected && isCorrectNetwork,
  });
      ...
      <CardContent className="space-y-6 p-6">
        ...
        {!isCorrectNetwork && isConnected && (
          <div className="p-4 rounded-md bg-yellow-900/80 text-yellow-300 border border-yellow-700 flex flex-col items-center space-y-3">
            ...
            <p className="text-sm text-center">Please switch to the Pepu Testnet to use staking features.</p>
            <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          ...
          <div className="p-3 border border-green-700/50 rounded-md"><div className="text-sm text-green-400">Total MFG Staked</div><div className="text-xl font-bold text-white">{formatNumber(totalStaked)}</div></div>
          <div className="p-3 border border-green-700/50 rounded-md"><div className="text-sm text-green-400">Your Staked MFG</div><div className="text-xl font-bold text-white">{formatNumber(userStakedAmount)}</div></div>
        ...
        <div className="text-center">
          <button
            onClick={handleClaim}
            disabled={isLoading || !isCorrectNetwork || parseFloat(pendingRewards) <= 0}
        ...
        <div className="border border-green-700/50 rounded-md p-4">
          ...
          {activeTab === 'stake' ? (
            <div className="space-y-4">
              ...
              {needsApproval && (<div className="flex items-center text-xs text-yellow-400 space-x-2"><Info size={16}/><span>Approval required before staking. This is a one-time transaction (or until revoked).</span></div>)}
            </div>
          ) : (
