...
export default function StakingSection() {
  ...
  // --- UI ---
  if (!isConnected) {
    return <button onClick={connectMetaMask} className="bg-blue-600 text-white px-4 py-2 rounded">Connect Wallet</button>;
  }

  if (!isTestnet && !isMainnet) {
    return <p>Please switch to PepU Testnet or Mainnet.</p>;
  }

  if (!STAKING_ADDRESS) {
    return <p>Staking contract is not deployed on this network yet.</p>;
  }
  ...
  return (
    <div className="p-4 border rounded-xl shadow-md space-y-4">
      ...
      {needsApproval ? (
        <button onClick={handleApprove} className="bg-blue-600 text-white px-4 py-2 rounded-xl">Approve</button>
      ) : (
        <button onClick={handleStake} className="bg-green-600 text-white px-4 py-2 rounded-xl">Stake</button>
      )}

      <div className="flex space-x-4">
        <button onClick={handleUnstake} className="bg-red-600 text-white px-4 py-2 rounded-xl">Unstake</button>
        <button onClick={handleClaim} className="bg-yellow-600 text-black px-4 py-2 rounded-xl">Claim Rewards</button>
      </div>
