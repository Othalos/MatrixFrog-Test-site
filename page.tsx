...
import { BarChart3, FileVideo, User, Database, Gamepad2, Star, Sprout } from "lucide-react";
...
const useVotingWalletBalances = (episodeId: string) => {
  ...
  const redWalletAddress = episode?.redWalletAddress || "0xe59c1a574a1f65e63cefe569588b1f5dc4b595e1";
  const greenWalletAddress = episode?.greenWalletAddress || "0xe7ca4d7dea1dfb2d7cab78a3763774fa81f64e7d";

  const { data: redPillBalance, isLoading: redPillLoading, refetch: refetchRedPill } = useReadContract({
  ...
  const { data: greenPillBalance, isLoading: greenPillLoading, refetch: refetchGreenPill } = useReadContract({
    address: PTX_TOKEN_ADDRESS,
    abi: ERC20_ABI,
    functionName: "balanceOf",
  ...
  if (!episodeId || !episode) {
    return {
      redPillVotes: 0, greenPillVotes: 0, totalVotes: 0,
      redPillBalance: "0", greenPillBalance: "0", isLoading: false,
      refetch: () => { refetchRedPill(); refetchGreenPill(); },
    };
  }

  const currentRedVotes = redPillBalance ? Number(formatUnits(redPillBalance as bigint, 18)) / 10000 : 0;
  const currentGreenVotes = greenPillBalance ? Number(formatUnits(greenPillBalance as bigint, 18)) / 10000 : 0;
  ...
  return {
    redPillVotes: Math.floor(currentRedVotes), greenPillVotes: Math.floor(currentGreenVotes),
...
const GamesSection = () => {
  ...
  const availableGames = [
    { value: "flap-matrix",   title: "Flap Matrix",          description: "Navigate the Matrix character through green candles and avoid obstacles", component: FlapMatrix },
    { value: "matrix-snake",  title: "Matrix Snake",          description: "Classic snake game with Matrix frog head - eat green dots and grow longer", component: MatrixSnake },
    { value: "pepu-memewars", title: "Pepu MemeWars By Jonny",description: "Click and hold to play this exciting meme battle game",                  component: PepuMemeWars },
    { value: "pepu-jump",     title: "Pepu Jump By Jonny",    description: "Tap in the direction you want to jump and navigate through challenges",    component: PepuJump },
  ];
    ...
    <div className="games-section-container">
      <Card style={{ backgroundColor: "black", border: "1px solid #22c55e", marginBottom: "24px" }}>
        <CardHeader style={{ padding: "24px" }}>
          <CardTitle style={{ color: "#4ade80", fontFamily: "monospace" }}>MATRIX GAMES</CardTitle>
        </CardHeader>
        <CardContent style={{ padding: "24px", paddingTop: "0" }}>
          <div className="games-dropdown-container" style={{ marginBottom: "24px" }}>
            <Select value={selectedGame} onValueChange={setSelectedGame}>
              <SelectTrigger style={{ width: "100%", backgroundColor: "black", border: "1px solid rgba(34,197,94,0.3)", color: "#4ade80", fontFamily: "monospace" }}>
                <SelectValue placeholder="Select Game" />
              </SelectTrigger>
              <SelectContent style={{ backgroundColor: "black", border: "1px solid rgba(34,197,94,0.3)" }}>
                {availableGames.map((game) => (
                  <SelectItem key={game.value} value={game.value} style={{ color: "#4ade80", paddingTop: "4px", paddingBottom: "4px", fontFamily: "monospace" }}>
                    {game.title}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          {currentGame && (
            <div>
              <div style={{ marginBottom: "20px", padding: "16px", backgroundColor: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: "4px" }}>
                <h3 style={{ color: "#4ade80", fontFamily: "monospace", fontSize: "1.1rem", margin: "0 0 8px 0" }}>{currentGame.title}</h3>
                <p style={{ color: "#86efac", fontFamily: "monospace", fontSize: "0.9rem", lineHeight: "1.5", margin: "0" }}>{currentGame.description}</p>
              </div>
...
const CollectiblesSection = () => (
  <div style={{ opacity: 0.6 }}>
    <Card style={{ backgroundColor: "black", border: "1px solid rgba(100,100,100,0.3)", marginBottom: "24px" }}>
      <CardHeader>
        <CardTitle style={{ color: "#6b7280", fontFamily: "monospace" }}>MATRIX COLLECTIBLES</CardTitle>
      </CardHeader>
      <CardContent>
        <div style={{ textAlign: "center", padding: "40px 20px" }}>
          <Star style={{ width: "48px", height: "48px", color: "#6b7280", margin: "0 auto 16px" }} />
          <h3 style={{ color: "#6b7280", fontFamily: "monospace", fontSize: "1.2rem", marginBottom: "12px" }}>COMING SOON</h3>
          ...
          <div style={{ marginTop: "24px", display: "flex", flexDirection: "column", gap: "8px", alignItems: "center" }}>
            {["🎨 Unique Digital Artifacts", "🔄 Peer-to-Peer Trading", "🏛️ Virtual Gallery Display"].map((item) => (
              <div key={item} style={{ padding: "8px 16px", backgroundColor: "rgba(100,100,100,0.1)", border: "1px solid rgba(100,100,100,0.3)", borderRadius: "4px", color: "#6b7280", fontSize: "0.8rem", fontFamily: "monospace" }}>
                {item}
              </div>
            ))}
          </div>
...
export default function MatrixConstruct() {
  const [selectedEpisode, setSelectedEpisode] = useState(() => {
    const activeEpisode = EPISODE_CONFIGS.find((ep) => ep.status === "active");
    const completedEpisodes = EPISODE_CONFIGS.filter((ep) => ep.status === "completed");
    const latestCompleted = completedEpisodes.length > 0 ? completedEpisodes[completedEpisodes.length - 1] : null;
    return activeEpisode?.id || latestCompleted?.id || EPISODE_CONFIGS[0]?.id || "episode-1";
  });
  const [selectedBlooper, setSelectedBlooper] = useState("blooper-3");
  ...
  return (
    <ClientOnly fallback={
      <div style={{ minHeight: "100vh", backgroundColor: "black", color: "#4ade80", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "monospace" }}>
        Loading dashboard...
      </div>
    }>
      <MatrixConstructContent
        selectedEpisode={selectedEpisode} setSelectedEpisode={setSelectedEpisode}
        selectedBlooper={selectedBlooper} setSelectedBlooper={setSelectedBlooper}
        selected={selected} setSelected={setSelected}
        activeSection={activeSection} setActiveSection={setActiveSection}
        videoError={videoError} setVideoError={setVideoError}
        isVoting={isVoting} setIsVoting={setIsVoting}
        voteSuccess={voteSuccess} setVoteSuccess={setVoteSuccess}
        voteError={voteError} setVoteError={setVoteError}
        isHydrated={isHydrated} setIsHydrated={setIsHydrated}
      />
    </ClientOnly>
  );
}
...
function MatrixConstructContent({
  selectedEpisode, setSelectedEpisode, selectedBlooper, setSelectedBlooper,
  selected, setSelected, activeSection, setActiveSection,
  videoError, setVideoError, isVoting, setIsVoting,
  voteSuccess, setVoteSuccess, voteError, setVoteError,
  isHydrated, setIsHydrated,
}: {
  selectedEpisode: string; setSelectedEpisode: (v: string) => void;
  selectedBlooper: string; setSelectedBlooper: (v: string) => void;
  selected: string | null; setSelected: (v: string | null) => void;
  activeSection: string; setActiveSection: (v: string) => void;
  videoError: string | null; setVideoError: (v: string | null) => void;
  isVoting: boolean; setIsVoting: (v: boolean) => void;
  voteSuccess: boolean; setVoteSuccess: (v: boolean) => void;
  voteError: string | null; setVoteError: (v: string | null) => void;
  isHydrated: boolean; setIsHydrated: (v: boolean) => void;
}) {
  const { isConnected, address, rawPtxBalance, refetchBalance } = useWalletConnect();
  const router = useRouter();

  useEffect(() => { setIsHydrated(true); }, [setIsHydrated]);

  const { redPillVotes, greenPillVotes, totalVotes, isLoading: votingStatsLoading, refetch: refetchVotingStats } =
    useVotingWalletBalances(selectedEpisode || "");
  ...
  useEffect(() => {
    if (isConfirmed && hash) {
      setVoteSuccess(true); setIsVoting(false); setVoteError(null);
      refetchBalance(); refetchVotingStats();
      setTimeout(() => { setVoteSuccess(false); }, 10000);
    }
  }, [isConfirmed, hash, refetchBalance, refetchVotingStats, setVoteSuccess, setIsVoting, setVoteError]);

  useEffect(() => {
    if (writeError) {
      setVoteError(writeError.message); setIsVoting(false);
      setTimeout(() => { setVoteError(null); }, 10000);
    }
  ...
  useEffect(() => {
    const checkVotingEnd = async () => {
      ...
      } catch (error) { console.error("Failed to check voting end:", error); }
    };
    checkVotingEnd();
  ...
  const handleVote = async () => {
    if (!isConnected || !address) { setVoteError("Please connect your wallet first to vote"); return; }
    if (!selected)                { setVoteError("Please select a pill option first"); return; }
    const episode = getEpisodeStatus(selectedEpisode);
    if (!episode || episode.status !== "active") { setVoteError("Voting is not active for this episode"); return; }
    const requiredAmount = parseEther("10000");
    if (!rawPtxBalance || (rawPtxBalance as bigint) < requiredAmount) {
      setVoteError("Insufficient PTX balance. You need at least 10000 PTX to vote."); return;
    }
    try {
      setIsVoting(true); setVoteError(null); setVoteSuccess(false);
      const receiverAddress = selected === "red" ? episode.redWalletAddress : episode.greenWalletAddress;
      await writeContract({ address: PTX_TOKEN_ADDRESS, abi: ERC20_ABI, functionName: "transfer", args: [receiverAddress, requiredAmount] });
    } catch (error) {
      console.error("Vote transaction failed:", error);
      setVoteError("Transaction failed. Please try again."); setIsVoting(false);
    }
  };
  ...
  const blooperVideos = [
    ...
    { value: "blooper-3", title: "Blooper 3: Third Episode Bloopers",  src: "https://www.youtube.com/embed/e7vshXc65aA?enablejsapi=1" },
  ];
  ...
  return (
    <div style={{ paddingBottom: "20px", minHeight: "100vh", backgroundColor: "black", color: "#4ade80", fontFamily: "monospace" }}>
      {/* HEADER */}
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px", borderBottom: "1px solid rgba(34,197,94,0.3)" }} className="construct-header">
        <div style={{ display: "flex", alignItems: "center", gap: "10px", paddingLeft: "10px", paddingRight: "10px" }}>
          <div>
            <div style={{ fontSize: "2.3em", fontWeight: "bold", textShadow: "0 0 10px rgba(0, 255, 0, 0.5)", color: "var(--matrix-text-light)" }} className="construct-title">
              THE CONSTRUCT
            </div>
            <div style={{ fontSize: "0.75rem", color: "var(--matrix-text-light)" }}>v2.0 access level: open access</div>
          </div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: "0.75rem", color: "#16a34a" }}>System Status</div>
          <div style={{ fontSize: "1.125rem", fontWeight: "bold", display: "flex", alignItems: "center", gap: "5px", color: "#4ade80" }} className="construct-status">
            ONLINE
          </div>
        </div>
      </header>
      ...
      <div style={{ display: "flex" }} className="construct-dashboard">
        {/* SIDEBAR */}
        <div style={{ width: "18rem", padding: "18px", borderRight: "1px solid rgba(34,197,94,0.3)", minHeight: "100vh" }} className="construct-sidebar">
          <nav style={{ padding: "5px", display: "flex", flexDirection: "column", gap: "8px" }}>
            {sidebarItems.map((item, index) => (
              <div
                key={index}
                onClick={() => { if (!item.disabled) { if (item.onClick) item.onClick(); if (item.href !== "#") router.push(item.href); } }}
                style={{
                  display: "flex", alignItems: "center", gap: "16px", padding: "12px", borderRadius: "8px",
                  border: item.active ? "1px solid #22c55e" : item.disabled ? "1px solid rgba(100,100,100,0.3)" : "1px solid rgba(34,197,94,0.3)",
                  backgroundColor: item.active ? "rgba(34,197,94,0.1)" : "transparent",
                  cursor: item.disabled ? "not-allowed" : "pointer",
                  transition: "all 0.3s ease",
                  opacity: item.disabled ? 0.5 : 1,
                }}
              >
                <item.icon style={{ width: "16px", height: "16px", color: item.disabled ? "#6b7280" : undefined }} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: "0.875rem", fontWeight: "500", color: item.disabled ? "#6b7280" : undefined }}>{item.label}</div>
                  <div style={{ fontSize: "0.75rem", color: item.disabled ? "#6b7280" : "#16a34a" }}>{item.subtitle}</div>
                </div>
              </div>
            ))}
          </nav>
                ...
                <Card style={{ backgroundColor: "black", border: "1px solid rgba(34,197,94,0.3)" }}>
                  <CardContent style={{ padding: "32px", textAlign: "center" }}>
                    <div className="video-container" style={{ width: "100%", margin: "0 auto", border: "2px solid #22c55e", borderRadius: "8px", overflow: "hidden" }}>
                      <iframe key={currentSagaEpisode?.id} width="100%" height="315" src={currentSagaEpisode?.videoUrl}
              ...
              <Card style={{ backgroundColor: "black", border: "1px solid rgba(34,197,94,0.3)", marginBottom: "24px" }}>
                ...
                <CardContent>
                  <p style={{ fontSize: "0.8rem", color: "#86efac", lineHeight: "1.6", paddingLeft: "16px", paddingRight: "16px" }}>
                    {(() => { const episode = getEpisodeStatus(selectedEpisode || "episode-4"); return episode?.description || "Episode description not available."; })()}
                  </p>
                </CardContent>
              </Card>

              {(() => {
                const episode = getEpisodeStatus(selectedEpisode || "episode-4");
                if (!episode) {
                  return (
                ...
                return (
                  <VotingSection
                    ...
                    isHydrated={isHydrated} isPending={isPending} isConfirming={isConfirming}
                    onVote={handleVote} redPillVotes={redPillVotes} greenPillVotes={greenPillVotes}
                    totalVotes={totalVotes} votingStatsLoading={votingStatsLoading}
                  />
                );
              })()}
            </>
            ...
            <>
              <div style={{ marginBottom: "24px" }} className="bloopers-section">
                <Card style={{ backgroundColor: "black", border: "1px solid rgba(34,197,94,0.3)" }}>
                  <CardContent style={{ padding: "32px", textAlign: "center" }}>
                    <div className="video-container" style={{ width: "100%", margin: "0 auto", border: "2px solid #22c55e", borderRadius: "8px", overflow: "hidden" }}>
                      <iframe key={selectedBlooper} width="100%" height="315"
